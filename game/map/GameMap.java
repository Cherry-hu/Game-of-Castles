...
public class GameMap {
    ...
    private void generateEdges() {
    	 List<Node<Castle>> c = castleGraph.getNodes();
    	 for(int i = 0; i < c.size()-1; i++) {
    		 Castle thiscastle = c.get(i).getValue();
    		 List<Castle> cc = (List<Castle>) castleGraph.getNodes().stream().map(x -> x.getValue()).collect(Collectors.toList());
    		 Collections.sort(cc, new Comparator<Castle>() {
    		 public int compare(Castle c1, Castle c2) {
    			 return (int)(c1.distance(thiscastle) - c2.distance(thiscastle));
    		 }
    	 });
    		 Castle ca = cc.get(1);
    		 Castle cb = cc.get(2);
    		 Castle cca = cc.get(3);
    		 Edge<Castle> e1 = castleGraph.addEdge(c.get(i), castleGraph.getNode(ca));
    		 Edge<Castle> e2 = castleGraph.addEdge(c.get(i), castleGraph.getNode(cb));
    		 Edge<Castle> e3 = castleGraph.addEdge(c.get(i), castleGraph.getNode(cca));
