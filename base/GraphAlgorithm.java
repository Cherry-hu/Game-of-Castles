...
public abstract class GraphAlgorithm<T> {
    ...
    private AlgorithmNode<T> getSmallestNode() {
    	...
    	if(availableNodes.size()!=0) {
    		Node<T> kleinsteNode=availableNodes.get(0);
    		AlgorithmNode<T> kleinsteAlgorithmNode=algorithmNodes.get(kleinsteNode);
    		for(Iterator<Node<T>> iter =availableNodes.iterator(); iter.hasNext();) {
    			Node<T> node=(Node<T>)iter.next();
    			AlgorithmNode<T> knote=algorithmNodes.get(node);
    			if(knote.value<kleinsteAlgorithmNode.value) {
    				kleinsteAlgorithmNode=knote;
    				kleinsteNode=node;
    			}
    		}
    		availableNodes.remove(kleinsteNode);
    		return kleinsteAlgorithmNode;
    	}
    	else {
    		return null;}
    }
    ...
    public void run() {
        // TODO: GraphAlgorithm<T>#run()
    	 ...
    	 while(v!=null) {
    		 List<Edge<T>> alleDirektKante=graph.getEdges(v.node);
    		 for(Edge<T> e: alleDirektKante) {
    			 if(isPassable(e)) {
    				 Node<T> n=e.getOtherNode(v.node);
    				 double valueVonN=algorithmNodes.get(n).value;
    				 double a=v.value+getValue(e);
    				 if((valueVonN==-1)||a<valueVonN) {
    					 algorithmNodes.put(n,new AlgorithmNode<>(n, v, a));
    				 }
    			 }
    		 }
    		 v=getSmallestNode(); 
    ...
    public List<Edge<T>> getPath(Node<T> destination) {
    	...
    	while(kante!=null) {
    		...
    		previous=Zielknoten.previous;
    		if(previous==null) {break;}
    		kante=graph.getEdge(previous.node,Zielknoten.node);
    	}
    	if(pfads.size()>0) {
    		Collections.reverse(pfads);
    		return pfads;
    	}
        return null;
    }
